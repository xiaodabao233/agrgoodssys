...
<!-- 添加地块的模态框 -->
<div class="modal fade" id="landInfoAddModal" tabindex="-1" role="dialog" aria-labelledby="myModalLabel">
...
<!-- 地块修改的模态框 -->
<div class="modal fade" id="landInfoUpdateModal" tabindex="-1" role="dialog" aria-labelledby="myModalLabel">
    <div class="modal-dialog" role="document">
        <div class="modal-content">
            <div class="modal-header">
                <button type="button" class="close" data-dismiss="modal" aria-label="Close"><span aria-hidden="true">&times;</span>
                </button>
                <h4 class="modal-title">地块修改</h4>
            </div>
            <div class="modal-body">
                <form class="form-horizontal">
                    <div class="form-group">
                        ...
                        <div class="col-md-12">
                            <table class="table table-hover" id="landinfo_table">
                                ...
                                <tr style="color: #ffffff">
                                    ...
                                    <th style="text-align:center">园区id</th>
                                    <th style="text-align:center">地块编号</th>
                                    <th style="text-align:center">地块名</th>
                                    <th style="text-align:center">东至</th>
                                    <th style="text-align:center">西至</th>
                                    <th style="text-align:center">南至</th>
                                    <th style="text-align:center">北至</th>
...
<script type="text/javascript">
    ...
    function to_page(pn) {
        $.ajax({
            url: "${APP_PATH}/landinfo-list",
            data: "pn=" + pn,
            type: "GET",
            success: function (result) {
                //1、解析并显示地块数据
                build_landInfo_table(result);
                //2、解析并显示分页信息
    ...
    function build_landInfo_table(result) {
        //清空table表格
        $("#landInfo_table tbody").empty();
        var landInfo = result.extend.pageInfo.list;
        $.each(landInfo, function (index, item) {
            var checkBoxTd = $("<td><input type='checkbox' class='check_item'/></td>");
            var landIdTd = $("<td></td>").append(item.landid);
            var parkIdTd = $("<td></td>").append(item.parkid);
            var landNumTd = $("<td></td>").append(item.landnum);
            var landNameTd = $("<td></td>").append(item.landname);
            var landETd = $("<td></td>").append(item.lande);
            var landWTd = $("<td></td>").append(item.landw);
    ...
    $("#landInfo_save_btn").click(function () {
        ...
        // 2、发送ajax请求保存菜地信息
        $.ajax({
            url: "${APP_PATH}/landinfo",
            type: "POST",
            data: $("#landInfoAddModal form").serialize(),
            success: function (result) {
                //菜地信息保存成功
                //1、关闭模态框
                $("#landInfoAddModal").modal('hide');
                //2、来到最后一页，显示刚才保存的数据
                //发送ajax请求显示最后一页数据即可
                to_page(totalPages);
            }
